        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Tactile-Language Grounding Debt: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study tactile-language-action models. The submission-hardened claim is intentionally narrow: Measure what language-conditioned policies fail to ground until touch resolves it. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Tactile language grounding debt should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.404 $\pm$ 0.010 & 0.445 $\pm$ 0.010 & 0.315 $\pm$ 0.015 \\
Data-augmented WAM & 0.495 $\pm$ 0.009 & 0.518 $\pm$ 0.009 & 0.411 $\pm$ 0.022 \\
Scalar uncertainty & 0.559 $\pm$ 0.016 & 0.578 $\pm$ 0.011 & 0.493 $\pm$ 0.017 \\
Ensemble disagreement & 0.586 $\pm$ 0.012 & 0.623 $\pm$ 0.014 & 0.533 $\pm$ 0.014 \\
Conformal risk filter & 0.619 $\pm$ 0.006 & 0.638 $\pm$ 0.015 & 0.554 $\pm$ 0.019 \\
Proposed branch mechanism & 0.688 $\pm$ 0.008 & 0.704 $\pm$ 0.017 & 0.645 $\pm$ 0.016 \\
Oracle branch upper bound & 0.762 $\pm$ 0.011 & 0.775 $\pm$ 0.022 & 0.727 $\pm$ 0.022 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.669 $\pm$ 0.014 & all components \\
Minus branch atlas & 0.571 $\pm$ 0.019 & removes explicit unrealized futures \\
Minus tail risk & 0.570 $\pm$ 0.022 & uses mean predicted value only \\
Minus diagnostic probe & 0.609 $\pm$ 0.020 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.619 $\pm$ 0.017 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/81.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Tactile-Language Grounding Debt is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
